...
## Installation
You can install it in your application using composer via command below:
```
...
```
### Custom Decoders
You can create your own custom decvoder (like HTML converter). All the decoders needs to implement `Converter\Decoders\Decoder` interface. You should implement the `handle` method in your converter which accepts the `$entity` object and `$data` parameter. Once you converted `$data` to a valid PHP array, you can use `Decorator::handle()`  to fill the entity properties with the incoming data:
```php
<?php  
  
namespace Converter\Decoders;  
  
use Converter\Decorator;
